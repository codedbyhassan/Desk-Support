...
export default function ManagementTab() {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-3">
      <Card className="p-5"><p className="text-sm text-muted-foreground">Total members</p><p className="mt-1 text-2xl font-semibold">{totalUsers}</p></Card>
      <Card className="p-5"><p className="text-sm text-muted-foreground">Active</p><p className="mt-1 text-2xl font-semibold">{activeUsers}</p></Card>
      <Card className="p-5"><p className="text-sm text-muted-foreground">Inactive</p><p className="mt-1 text-2xl font-semibold">{inactiveUsers}</p></Card>
    </div>

    <Card className="overflow-hidden">
      <div className="flex flex-col gap-4 border-b p-5 lg:flex-row lg:items-center lg:justify-between">
        <div><h2 className="text-lg font-semibold">People</h2><p className="text-sm text-muted-foreground">Select a member to view their profile, access and account actions.</p></div>
        <Button onClick={()=>setInviteOpen(true)}><UserPlus className="mr-2 h-4 w-4"/>Invite member</Button>
      </div>
      <div className="flex flex-col gap-3 border-b p-4 sm:flex-row">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/><Input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search username, people, roles or departments..." className="pl-9"/></div>
        <Tabs value={tab} onValueChange={v=>setTab(v as typeof tab)}><TabsList><TabsTrigger value="all">All</TabsTrigger><TabsTrigger value="active">Active</TabsTrigger><TabsTrigger value="inactive">Inactive</TabsTrigger></TabsList></Tabs>
      </div>
      {loading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin"/></div> : visible.length===0 ? <div className="flex min-h-64 flex-col items-center justify-center gap-2 p-8 text-center"><Users className="h-8 w-8 text-muted-foreground"/><p className="font-medium">No members found</p><p className="text-sm text-muted-foreground">Try changing your search or filter.</p></div> : <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
        {visible.map(m=>{const name=m.profile?.full_name??'Unnamed user';const username=m.profile?.username??'unknown';const role=ROLES.find(r=>r.value===m.role)?.label??m.role;return <button key={m.id} type="button" onClick={()=>navigate(`/app/users/${m.id}`)} className="group text-left">
          <Card className="h-full overflow-hidden border-border/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-border hover:shadow-md">
            <div className="relative h-28 bg-muted/50"><div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-background/60 to-transparent"/><div className="absolute right-4 top-4"><Badge variant={m.is_active?'secondary':'outline'}>{m.is_active?'Active':'Inactive'}</Badge></div></div>
            <div className="relative px-5 pb-5">
              <Avatar className="-mt-10 h-20 w-20 border-4 border-card shadow-sm"><AvatarFallback className="text-lg">{initials(name)}</AvatarFallback></Avatar>
              <div className="mt-4 flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-base font-semibold">{name}</h3><p className="mt-0.5 truncate text-sm text-muted-foreground">@{username}</p></div><ArrowUpRight className="mt-1 h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5"/></div>
              <div className="mt-5 grid grid-cols-2 gap-3 border-t pt-4"><div><p className="text-[11px] uppercase tracking-wide text-muted-foreground">Role</p><p className="mt-1 truncate text-sm font-medium">{role}</p></div><div><p className="text-[11px] uppercase tracking-wide text-muted-foreground">Department</p><p className="mt-1 truncate text-sm font-medium">{m.department?.name??'No department'}</p></div></div>
            </div>
          </Card>
        </button>})}
      </div>}
