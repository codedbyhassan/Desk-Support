...
export default function ManagementTab() {
  ...
  const invite = async (event:React.FormEvent)=>{event.preventDefault();if(!companyId)return;setSaving(true);try{const {data,error}=await supabase.functions.invoke('invite-user',{body:{company_id:companyId,email:form.email,full_name:form.full_name,role:form.role,department_id:form.department_id==='none'?null:form.department_id,phone:form.phone||null}});if(error)throw error;if(!data?.ok)throw new Error(data?.error??'Invitation failed');toast({title:'Invitation sent',description:`${form.full_name} can now finish their account setup.`});setInviteOpen(false);setForm({email:'',full_name:'',role:'employee',department_id:'none',phone:''});await load()}catch(error:any){toast({title:'Invitation failed',description:error.message??'Please try again.',variant:'destructive'})}finally{setSaving(false)}}
  const saveEdit=async()=>{if(!companyId||!edit)return;setSaving(true);try{const {data,error}=await supabase.functions.invoke('provision-user',{body:{company_id:companyId,user_id:edit.user_id,full_name:edit.profile?.full_name??'User',role:edit.role,department_id:edit.department_id}});if(error)throw error;if(!data?.ok)throw new Error(data?.error??'Update failed');toast({title:'Membership updated'});setEdit(null);await load()}catch(error:any){toast({title:'Update failed',description:error.message??'Please try again.',variant:'destructive'})}finally{setSaving(false)}}
  const toggleStatus=async()=>{if(!companyId||!statusTarget)return;setSaving(true);try{const {data,error}=await supabase.functions.invoke('manage-user-status',{body:{company_id:companyId,user_id:statusTarget.user_id,is_active:!statusTarget.is_active}});if(error)throw error;if(!data?.ok)throw new Error(data?.error??'Status update failed');toast({title:statusTarget.is_active?'Member deactivated':'Member reactivated'});setStatusTarget(null);await load()}catch(error:any){toast({title:'Status update failed',description:error.message??'Please try again.',variant:'destructive'})}finally{setSaving(false)}}
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={inviteOpen} onOpenChange={setInviteOpen}><DialogContent><DialogHeader><DialogTitle>Invite a team member</DialogTitle><DialogDescription>Send an email invitation. The user creates their own password during setup.</DialogDescription></DialogHeader><form onSubmit={invite} className="space-y-4"><div><Label>Full name</Label><Input required value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})}/></div><div><Label>Email</Label><Input required type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></div><div><Label>Phone</Label><Input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></div><div className="grid gap-4 sm:grid-cols-2"><div><Label>Role</Label><Select value={form.role} onValueChange={v=>setForm({...form,role:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{ROLES.map(r=><SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent></Select></div><div><Label>Department</Label><Select value={form.department_id} onValueChange={v=>setForm({...form,department_id:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="none">No department</SelectItem>{departments.map(d=><SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent></Select></div></div><DialogFooter><Button type="button" variant="outline" onClick={()=>setInviteOpen(false)}>Cancel</Button><Button disabled={saving} type="submit">{saving&&<Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Send invitation</Button></DialogFooter></form></DialogContent></Dialog>
    <Dialog open={!!edit} onOpenChange={o=>!o&&setEdit(null)}><DialogContent>{edit&&<><DialogHeader><DialogTitle>Edit membership</DialogTitle><DialogDescription>Change this member's company role or department.</DialogDescription></DialogHeader><div className="space-y-4"><div><Label>Role</Label><Select value={edit.role} onValueChange={v=>setEdit({...edit,role:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{ROLES.map(r=><SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent></Select></div><div><Label>Department</Label><Select value={edit.department_id??'none'} onValueChange={v=>setEdit({...edit,department_id:v==='none'?null:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="none">No department</SelectItem>{departments.map(d=><SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent></Select></div></div><DialogFooter><Button variant="outline" onClick={()=>setEdit(null)}>Cancel</Button><Button disabled={saving} onClick={saveEdit}>{saving&&<Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Save changes</Button></DialogFooter></>}</DialogContent></Dialog>
