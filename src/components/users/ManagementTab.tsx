...
type Membership = {
  ...
  profile: { id:string; username:string; full_name: string; avatar_url: string | null; phone: string | null } | null
    ...
    try {
      const [members, depts, counts] = await Promise.all([
        fetchSupabasePage<Membership>('company_memberships', 0, {
          ...
          columns:'id,user_id,company_id,role,department_id,is_active,joined_at,profiles!company_memberships_user_id_fkey(id,username,full_name,avatar_url,phone),departments!company_memberships_department_id_fkey(name)',
