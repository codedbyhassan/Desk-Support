...
export function Header({ navItems, mobileMenuOpen, setMobileMenuOpen }: HeaderProps) {
  ...
  return <header className="sticky top-0 z-40 h-[72px] border-b border-border bg-background">
    <div className="relative flex h-full items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute left-1/2 top-1/2 flex h-9 w-[min(52vw,420px)] -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 text-sm text-muted-foreground transition-colors focus-within:border-primary/40 focus-within:bg-background">
        <Search className="h-4 w-4 shrink-0"/>
        <span className="truncate">Search workspace</span>
        <kbd className="ml-auto hidden shrink-0 rounded border border-border bg-background px-1.5 py-0.5 text-[10px] sm:inline-flex">⌘K</kbd>
      </div>
