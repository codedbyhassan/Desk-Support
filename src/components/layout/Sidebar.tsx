  ...
  return <aside className="sticky top-0 hidden h-screen w-[248px] flex-col border-r border-border bg-card lg:flex">
    <div className="flex h-[72px] shrink-0 items-center border-b border-border px-5">
      <button onClick={() => navigate('/app/dashboard')} className="flex items-center gap-3 text-left">
        ...
        <span><span className="block text-[16px] font-bold tracking-tight">Desk-Support</span><span className="mt-0.5 block text-[11px] text-muted-foreground">Service workspace</span></span>
      </button>
    </div>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-6">
      ...
      <nav className="space-y-1">
        {mainItems.map(item => {
          ...
          return <button key={item.id} onClick={() => navigate(item.href)} aria-current={active ? 'page' : undefined} className={`relative flex h-11 w-full items-center gap-3 rounded-lg px-3 text-[14px] font-medium transition-colors ${active ? 'bg-primary/10 font-semibold text-primary' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>
            ...
            {item.badge && <span className="rounded-full bg-primary px-1.5 py-0.5 text-[11px] font-bold text-primary-foreground">{item.badge}</span>}
