        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 lg:gap-6">
          {loading ? (
            ...
            filteredDepartments.map((dept) => (
              <Card
                key={dept.id}
                className="p-3 lg:p-6 cursor-pointer hover:shadow-lg transition-all duration-200 relative overflow-hidden group border-slate-200"
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="flex items-start gap-3 lg:block">
                    {/* Icon */}
                    <div className="h-12 w-12 lg:h-12 lg:w-12 rounded-xl bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center shadow-lg shadow-orange-500/30 flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0 lg:mt-4">
                      ...
                      <div className="flex items-start justify-between gap-2 mb-1.5 lg:mb-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-sm lg:text-lg truncate mb-0.5">{dept.name}</h3>
                        ...
                        {dept._count && dept._count.pending_tickets > 0 && (
                          <Badge variant="destructive" className="text-[9px] lg:text-xs animate-pulse flex-shrink-0">
                      ...
                      {/* Description */}
                      <p className="text-[10px] lg:text-sm text-gray-600 line-clamp-2 leading-relaxed mb-2 lg:mb-3">
                        {dept.description}
                      </p>

                      {/* Stats - Horizontal on Mobile */}
                      <div className="flex items-center gap-3 lg:grid lg:grid-cols-3 lg:gap-4 pt-2 lg:pt-4 border-t border-slate-100">
                        <div className="flex items-center gap-1.5 lg:flex-col lg:text-center">
                          <Users className="h-3.5 w-3.5 lg:h-4 lg:w-4 text-gray-600" />
                          <div className="lg:mt-1">
                            <p className="text-sm lg:text-2xl font-bold text-gray-900">
                              {dept._count?.members || 0}
                            </p>
                            <p className="text-[9px] lg:text-xs text-gray-500 hidden lg:block">Members</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 lg:flex-col lg:text-center">
                          <Clock className="h-3.5 w-3.5 lg:h-4 lg:w-4 text-orange-600" />
                          <div className="lg:mt-1">
                            <p className="text-sm lg:text-2xl font-bold text-orange-600">
                              {dept._count?.pending_tickets || 0}
                            </p>
                            <p className="text-[9px] lg:text-xs text-gray-500 hidden lg:block">Pending</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 lg:flex-col lg:text-center">
                          <Ticket className="h-3.5 w-3.5 lg:h-4 lg:w-4 text-blue-600" />
                          <div className="lg:mt-1">
                            <p className="text-sm lg:text-2xl font-bold text-blue-600">
                              {dept._count?.tickets || 0}
                            </p>
                            <p className="text-[9px] lg:text-xs text-gray-500 hidden lg:block">Total</p>
                          </div>
                        </div>
                      </div>
