...
import { Building2, CheckCircle, Clock, Edit3, Lock, Mail, QrCode, Save, Upload, User, X } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await updateProfile({ full_name: fullName.trim(), phone: phone.trim() || null, department_id: departmentId || null, avatar_url: avatarUrl })
      if (newPassword) {
        const { error } = await supabase.auth.updateUser({ password: newPassword })
        if (error) throw error
      }
      toast({ title: 'Profile saved', description: 'Your profile changes were saved successfully.' })
      setEditing(false); setNewPassword(''); setConfirmPassword(''); setAvatarFile(null)
    } catch (error) {
  ...
  function cancelEdit() {
    ...
    setAvatarPreview(user.avatar_url ?? ''); setAvatarFile(null); setNewPassword(''); setConfirmPassword(''); setErrors({}); setEditing(false)
  }
