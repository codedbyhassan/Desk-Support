...
import { Award, Building2, CheckCircle, Clock, Edit3, Lock, Mail, Phone, QrCode, Save, Shield, Upload, User, X } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  useQRScanner({
    ...
    onScanSuccess: async (data) => {
      try {
        ...
        toast({ title: 'Attendance updated', description: 'Your attendance status has been recorded.' })
      } catch (error) {
  ...
  const attendanceSummary = useMemo(() => {
    ...
    const worked = (records: AttendanceRecord[]) => records.filter((record) => record.status === 'clocked_in' || record.status === 'clocked_out')
    const hours = (records: AttendanceRecord[]) => records.reduce((total, record) => {
      if (!record.check_in || !record.check_out) return total
      const value = (new Date(record.check_out).getTime() - new Date(record.check_in).getTime()) / 3600000
    ...
    return {
      ...
      weekPresent: worked(week).length,
      weekHours: hours(week),
      monthPresent: worked(month).length,
      monthHours: hours(month),
      yearPresent: worked(year).length,
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        const { error: uploadError } = await supabase.storage.from('photos').upload(path, avatarFile, { upsert: true })
      ...
      <Card className="overflow-hidden border border-border bg-card shadow-sm">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-4">
              <Avatar className="h-16 w-16 shrink-0 border-2 border-background shadow-sm sm:h-20 sm:w-20">
                ...
                <AvatarFallback className="bg-primary text-xl font-bold text-primary-foreground">{user.full_name.slice(0, 2).toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="truncate text-xl font-bold tracking-tight text-foreground sm:text-2xl">{user.full_name}</h1>
                  <Badge variant="secondary">{user.role}</Badge>
                </div>
                <p className="mt-1 flex items-center gap-2 truncate text-sm text-muted-foreground"><Mail className="h-4 w-4 shrink-0" />{user.email}</p>
                {department && <p className="mt-1 flex items-center gap-2 text-sm text-muted-foreground"><Building2 className="h-4 w-4 shrink-0" />{department.name}</p>}
              </div>
            </div>
            {!editing && <Button onClick={() => setEditing(true)}><Edit3 className="mr-2 h-4 w-4" />Edit profile</Button>}
          </div>
        </div>
        ...
        <Tabs defaultValue="profile" className="w-full">
          ...
          <TabsContent value="profile" className="m-0">
            {editing ? (
              <form onSubmit={handleSave} className="bg-card">
                <div className="space-y-8 p-5 sm:p-6 lg:p-8">
                  <section className="space-y-5">
                    <div className="flex items-center gap-3 border-b border-border pb-4">
                      ...
                      <div><h2 className="font-semibold text-foreground">Personal information</h2><p className="text-sm text-muted-foreground">Keep your profile details up to date.</p></div>
                    </div>

                    <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-muted/20 p-5 sm:flex-row">
                      <Avatar className="h-20 w-20 border-2 border-background shadow-sm">
                        <AvatarImage src={avatarPreview} />
                        <AvatarFallback className="bg-primary text-primary-foreground">{fullName.slice(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div className="text-center sm:text-left">
                        <label htmlFor="avatar-upload" className="inline-flex h-10 cursor-pointer items-center rounded-xl border border-input bg-card px-4 text-sm font-medium shadow-sm hover:bg-muted">
                          <Upload className="mr-2 h-4 w-4" />Change photo
                          <input id="avatar-upload" type="file" accept="image/png,image/jpeg,image/gif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) { setAvatarFile(file); setAvatarPreview(URL.createObjectURL(file)) } }} disabled={saving} />
                        </label>
                        <p className="mt-2 text-xs text-muted-foreground">JPG, PNG or GIF. Keep the file under 5 MB.</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                      <div className="space-y-2"><Label htmlFor="profile-full-name">Full name</Label><Input id="profile-full-name" value={fullName} onChange={(e) => setFullName(e.target.value)} disabled={saving} aria-invalid={!!errors.fullName} />{errors.fullName && <p className="text-sm text-destructive">{errors.fullName}</p>}</div>
                      <div className="space-y-2"><Label htmlFor="profile-email">Email</Label><Input id="profile-email" value={user.email} disabled className="bg-muted" /><p className="text-xs text-muted-foreground">Email is managed by authentication.</p></div>
                      <div className="space-y-2"><Label htmlFor="profile-phone">Phone number</Label><Input id="profile-phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} disabled={saving} aria-invalid={!!errors.phone} placeholder="+233 20 000 0000" />{errors.phone && <p className="text-sm text-destructive">{errors.phone}</p>}</div>
                      <div className="space-y-2"><Label htmlFor="profile-department">Department</Label><Select value={departmentId || 'none'} onValueChange={(value) => setDepartmentId(value === 'none' ? '' : value)} disabled={saving}><SelectTrigger id="profile-department"><SelectValue placeholder="Select department" /></SelectTrigger><SelectContent><SelectItem value="none">No department</SelectItem>{departments.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></div>
                    </div>
                  </section>

                  <section className="space-y-5 border-t border-border pt-7">
                    <div className="flex items-center gap-3 border-b border-border pb-4"><div className="grid h-10 w-10 place-items-center rounded-xl bg-amber-500/10 text-amber-600"><Lock className="h-5 w-5" /></div><div><h2 className="font-semibold text-foreground">Password</h2><p className="text-sm text-muted-foreground">Leave both fields empty to keep your current password.</p></div></div>
