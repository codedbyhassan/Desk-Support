...
const EMPTY_COUNTS: ExactCompanyCounts = {
  ...
  tickets_closed: 0, tickets_unresolved: 0, tickets_overdue: 0, assets_total: 0, asset_assignments_active: 0,
  ticket_assignments_active: 0, ticket_comments_total: 0, ticket_attachments_total: 0, workspace_folders_total: 0,
  workspace_files_total: 0, notifications_unread: 0, attendance_today: 0, qr_codes_active: 0, qr_scans_today: 0,
...
export default function DashboardPage() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try { setError(null); setCounts(await getExactCompanyCounts(user.company_id)); await fetchAnalytics() }
  ...
  useEffect(() => {
    ...
    Promise.all([getExactCompanyCounts(user.company_id), fetchAnalytics()])
      .then(([nextCounts]) => { if (!cancelled) setCounts(nextCounts) })
      .catch(err => { if (!cancelled) setError(err instanceof Error ? err.message : 'Unable to load dashboard') })
      .finally(() => { if (!cancelled) setLoading(false) })
    const channel = supabase.channel(`dashboard-${user.company_id}`)
  ...
  return <div className="space-y-6">
    <div className="flex justify-end gap-2">
      <Button variant="outline" onClick={() => navigate('/app/notifications')} className="h-9 rounded-lg"><Bell className="mr-2 h-4 w-4" />Notifications{counts.notifications_unread > 0 && <span className="ml-2 rounded-full bg-primary px-1.5 text-[10px] text-primary-foreground">{counts.notifications_unread}</span>}</Button>
      <Button onClick={() => navigate('/app/tickets/new')} className="h-9 rounded-lg"><Plus className="mr-2 h-4 w-4" />New ticket</Button>
    </div>
