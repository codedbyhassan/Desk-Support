  ...
  return (
    <main className="marketing-page">
      <nav className="marketing-nav">
        ...
        {mobileOpen && <div className="marketing-container marketing-mobile-menu"><div>
      ...
      <section className="marketing-hero">
        <div className="marketing-container marketing-hero-grid">
          <div className="marketing-hero-copy-wrap">
            <div className="marketing-eyebrow hero-reveal hero-reveal-delay-1">IT operations, without the clutter</div>
            <h1 className="hero-reveal hero-reveal-delay-2">Run your support desk with <span>clarity.</span></h1>
            <p className="marketing-hero-copy hero-reveal hero-reveal-delay-3">Desk-Support brings tickets, assets, teams and operational activity into one focused workspace built for organisations that need work to stay visible.</p>
            <div className="marketing-hero-actions hero-reveal hero-reveal-delay-4">
              <button className="marketing-button marketing-button-primary marketing-magnetic" onClick={() => go('/signup')}>Create your workspace <ArrowRight size={16} /></button>
              <a className="marketing-button marketing-button-secondary" href="#workflow">See how it works <ChevronDown size={15} /></a>
            </div>
            <div className="marketing-trust hero-reveal hero-reveal-delay-5">Built around the work your support team already does — not another generic dashboard.</div>
          </div>
          <div className="marketing-product-frame marketing-parallax" aria-label="Desk-Support product preview">
      ...
      <section id="capabilities" className="marketing-section marketing-section-soft">
        <div className="marketing-container">
          <div className="marketing-section-heading reveal-up" data-reveal><small>Core capabilities</small><h2>One system for the work behind every request.</h2><p>Desk-Support connects the operational pieces that normally live across spreadsheets, chat threads and disconnected tools.</p></div>
          <div className="marketing-capabilities">{capabilities.map(({ icon: Icon, title, text }, index) => <article className="marketing-capability reveal-up" data-reveal style={{ '--reveal-delay': `${index * 70}ms` } as React.CSSProperties} key={title}><div className="marketing-capability-icon"><Icon size={19} /></div><h3>{title}</h3><p>{text}</p></article>)}</div>
        </div>
      </section>
      ...
      <section id="workflow" className="marketing-section">
        <div className="marketing-container marketing-workflow">
          <div>
            <div className="marketing-section-heading reveal-up" data-reveal style={{ '--reveal-delay': '80ms' } as React.CSSProperties}><small>How it works</small><h2>Less chasing. More resolving.</h2><p>Give every request a clear owner, every asset a record and every team a shared operational picture.</p></div>
            <div className="marketing-workflow-list">{[['01', 'Capture the request', 'Create a ticket with the context your team needs from the start.'], ['02', 'Assign the work', 'Route the request to the right person, team or department.'], ['03', 'Track the outcome', 'Keep activity, status and resolution history visible until the work is complete.']].map(([step, title, text], index) => <div className="marketing-workflow-item reveal-up" data-reveal style={{ '--reveal-delay': `${140 + index * 90}ms` } as React.CSSProperties} key={step}><div className="marketing-step">{step}</div><div><h3>{title}</h3><p>{text}</p></div></div>)}</div>
          </div>
          <div className="marketing-feature-panel reveal-scale" data-reveal><span className="label">Operational foundation</span><h3>Make the state of your organisation obvious.</h3><p>The goal is not more screens. It is a dependable source of truth for what needs attention, who owns it and what happened.</p><ul>{['Tickets connected to people and teams', 'Assets with accountable ownership', 'Role-aware operational access', 'A workspace designed for daily use'].map(item => <li key={item}><Check size={15} color="#93c5fd" />{item}</li>)}</ul></div>
        </div>
      </section>

      <section id="pricing" className="marketing-section marketing-section-soft">
        <div className="marketing-container">
          <div className="marketing-section-heading reveal-up" data-reveal><small>Pricing</small><h2>Start with the workflow. Scale when you need to.</h2><p>Simple plans for teams at different stages of operational maturity. Contact us when your requirements become more specific.</p></div>
          <div className="marketing-pricing">{plans.map((plan, index) => <article className={`marketing-price-card reveal-up ${plan.featured ? 'featured' : ''}`} data-reveal style={{ '--reveal-delay': `${index * 100}ms` } as React.CSSProperties} key={plan.name}><h3>{plan.name}</h3><div className="marketing-price">{plan.price}{plan.price !== 'Free' && <span> / plan</span>}</div><p>{plan.text}</p><ul>{plan.features.map(feature => <li key={feature}><Check size={15} color="#2563eb" />{feature}</li>)}</ul><button className={`marketing-button ${plan.featured ? 'marketing-button-primary' : 'marketing-button-secondary'}`} onClick={() => go('/signup')}>{plan.price === 'Free' ? 'Get started' : 'Talk to us'}</button></article>)}</div>
        </div>
      </section>

      <section className="marketing-cta"><div className="marketing-container marketing-cta-inner reveal-up" data-reveal><div><h2>Your support operation deserves a system built around it.</h2><p>Bring your tickets, assets and teams into one workspace.</p></div><button className="marketing-button marketing-button-primary marketing-magnetic" onClick={() => go('/signup')}>Get started <ArrowRight size={16} /></button></div></section>
