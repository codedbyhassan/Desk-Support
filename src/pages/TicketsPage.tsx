...
export default function TicketsPage({ newTicket = false }: TicketsPageProps) {
  ...
  useEffect(() => {
    if (user?.company_id) fetchTickets()
  ...
  const handleDeleteClick = (ticketId: string, event: MouseEvent) => {
  ...
  const highPriorityTickets = tickets.filter(
    ticket => ticket.priority === 'high' && !['resolved', 'closed'].includes(ticket.status),
  ).length
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={toggleForm} className="h-9 rounded-lg">
      ...
      <Card className="overflow-hidden border-border bg-card shadow-none">
        <div className="border-b border-border p-4 sm:p-5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-semibold">{activeTab === 'incoming' ? 'Incoming tickets' : 'My tickets'}</h2>
              <p className="text-xs text-muted-foreground">
                {displayedTickets.length} matching {displayedTickets.length === 1 ? 'ticket' : 'tickets'}
              </p>
            </div>
            <div className="inline-flex w-fit rounded-lg border border-border bg-muted/50 p-1">
              <button
                onClick={() => setActiveTab('incoming')}
                className={`rounded-md px-3 py-1.5 text-xs font-medium ${activeTab === 'incoming' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}
              >
                Assigned to me <span className="ml-1 opacity-60">{departmentTickets.length}</span>
              </button>
              <button
                onClick={() => setActiveTab('outgoing')}
                className={`rounded-md px-3 py-1.5 text-xs font-medium ${activeTab === 'outgoing' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}
              >
                Created by me <span className="ml-1 opacity-60">{personalTickets.length}</span>
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <div className="flex gap-2">
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="h-10 w-[145px] rounded-lg">
              ...
              user && (user.role === 'admin' || user.role === 'manager')
                ? ticket => (
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={event => handleDeleteClick(ticket.id, event)}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the ticket, comments and status history. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
