...
export default function TicketsPage({ newTicket = false }: TicketsPageProps) {
  ...
  const toggleForm = () => { if (showForm) { setShowForm(false); if (location.pathname === '/app/tickets/new') navigate('/app/tickets') } else { setShowForm(true); navigate('/app/tickets/new') } }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast({ title: 'Ticket deleted', description: 'The ticket and its history were removed.' }); fetchTickets()
    } catch (error) { console.error('Error deleting ticket:', error); toast({ title: 'Could not delete ticket', description: 'Please try again.', variant: 'destructive' }) }
    finally { setDeleting(false); setDeleteDialogOpen(false); setTicketToDelete(null) }
  }
  const matchesFilters = (ticket: typeof tickets[number]) => { const q = searchQuery.trim().toLowerCase(); const matchesSearch = !q || ticket.title?.toLowerCase().includes(q) || ticket.description?.toLowerCase().includes(q) || ticket.id?.toLowerCase().includes(q); return matchesSearch && (statusFilter === 'all' || ticket.status === statusFilter) && (priorityFilter === 'all' || ticket.priority === priorityFilter) }
