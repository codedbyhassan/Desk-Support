...
function hexToHsl(hex:string){const rgb=hexToRgb(hex);if(!rgb)return null;const r=rgb.r/255,g=rgb.g/255,b=rgb.b/255,max=Math.max(r,g,b),min=Math.min(r,g,b);let h=0,s=0;const l=(max+min)/2;if(max!==min){const d=max-min;s=l>.5?d/(2-max-min):d/(max+min);switch(max){case r:h=(g-b)/d+(g<b?6:0);break;case g:h=(b-r)/d+2;break;default:h=(r-g)/d+4}}return `${Math.round(h*60)} ${Math.round(s*100)}% ${Math.round(l*100)}%`}
...
export function ThemeProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>{if(!company?.id||localStorage.getItem('theme'))return;let cancelled=false;void supabase.from('company_settings').select('default_theme').eq('company_id',company.id).maybeSingle().then(({data})=>{if(cancelled)return;const value=data?.default_theme;if(value==='light'||value==='dark'||value==='system')setThemeState(value)});return()=>{cancelled=true}},[company?.id])
 useEffect(()=>{const root=document.documentElement;const apply=()=>{root.classList.remove('light','dark');root.classList.add(theme==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):theme);root.classList.toggle('high-contrast',themeSettings.highContrast);root.style.setProperty('--font-size-base',`${Math.min(20,Math.max(14,themeSettings.fontSize))}px`);root.style.setProperty('--radius',`${Math.min(16,Math.max(4,themeSettings.borderRadius))}px`);root.style.setProperty('--space-multiplier',String({compact:.75,default:1,comfortable:1.25}[themeSettings.componentSpacing]));root.style.setProperty('--transition-fast',themeSettings.reduceMotion?'0.01ms':'100ms');root.style.setProperty('--transition-normal',themeSettings.reduceMotion?'0.01ms':'150ms');root.style.setProperty('--transition-slow',themeSettings.reduceMotion?'0.01ms':'200ms')};apply();const media=matchMedia('(prefers-color-scheme: dark)');media.addEventListener?.('change',apply);localStorage.setItem('theme',theme);return()=>media.removeEventListener?.('change',apply)},[theme,themeSettings])
 useEffect(()=>{
   const primary=settings?.primary_color||customTheme.primary
   const secondary=settings?.secondary_color||customTheme.secondary
   const accent=settings?.accent_color||customTheme.accent
   const tokens:{name:string;value:string}[]=[
     {name:'primary',value:primary},{name:'secondary',value:secondary},{name:'accent',value:accent},{name:'ring',value:primary},{name:'sidebar-primary',value:primary},{name:'accent-foreground',value:accent}
   ]
   tokens.forEach(({name,value})=>{const hsl=hexToHsl(value);if(hsl)document.documentElement.style.setProperty(`--${name}`,hsl)})
   document.documentElement.style.setProperty('--color-primary',primary);document.documentElement.style.setProperty('--color-secondary',secondary);document.documentElement.style.setProperty('--color-accent',accent)
