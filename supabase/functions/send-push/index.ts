...
Deno.serve(async (req) => {
  try {
    ...
    let query = db.from('notification_devices').select('id,user_id,platform').eq('user_id', user.id)
    ...
    return json({ ok: true, configured: Boolean(Deno.env.get('VAPID_PRIVATE_KEY')), device_count: devices?.length ?? 0, devices: (devices ?? []).map(d => ({ id: d.id, platform: d.platform })) })
