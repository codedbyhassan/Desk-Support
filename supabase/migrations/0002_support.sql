...
create type ticket_channel as enum ('portal','email','phone','chat','other');
create type ticket_comment_type as enum ('public','internal');

-- The foundation helper returns SETOF uuid, which PostgreSQL does not allow directly
-- inside a policy expression. Keep it intact for compatibility and expose an array form
-- for RLS predicates.
create function public.current_company_id_array()
returns uuid[]
language sql
stable
security definer
...
create table public.tickets (
  ...
  constraint tickets_acceptance_consistency check ((accepted_at is null and accepted_by is null) or (accepted_at is not null and accepted_by is not null)),
  constraint tickets_resolution_consistency check ((status in ('resolved','closed') and resolved_at is not null) or status not in ('resolved','closed')),
  constraint tickets_closed_consistency check ((status = 'closed' and closed_at is not null) or (status <> 'closed' and closed_at is null))
);
...
create table public.ticket_assignments (
  ...
  note text,
  constraint ticket_assignments_time_check check (unassigned_at is null or unassigned_at >= assigned_at)
);
create unique index ticket_assignments_one_active_idx on public.ticket_assignments(ticket_id) where unassigned_at is null;
...
create table public.ticket_status_history (
  ...
  constraint ticket_status_history_transition_check check (from_status is null or from_status <> to_status)
);

alter table public.tickets add constraint tickets_category_same_company_fk
  foreign key (company_id, category_id)
  references public.ticket_categories(company_id, id) on delete set null;

create index idx_ticket_categories_company_active on public.ticket_categories(company_id, is_active);
create index idx_tickets_company_status on public.tickets(company_id, status);
create index idx_tickets_company_priority on public.tickets(company_id, priority);
create index idx_tickets_company_created_at on public.tickets(company_id, created_at desc);
create index idx_tickets_requester on public.tickets(requester_id, created_at desc);
create index idx_tickets_category on public.tickets(category_id);
create index idx_tickets_due_at on public.tickets(company_id, due_at) where due_at is not null and status not in ('resolved','closed');
create index idx_ticket_assignments_assignee_active on public.ticket_assignments(assignee_id, assigned_at desc) where unassigned_at is null;
create index idx_ticket_assignments_ticket_history on public.ticket_assignments(ticket_id, assigned_at desc);
...
create index idx_ticket_status_history_ticket_changed on public.ticket_status_history(ticket_id, changed_at desc);
...
create trigger ticket_comments_set_updated_at before update on public.ticket_comments for each row execute function public.set_updated_at();
...
comment on table public.ticket_status_history is 'Status transition history for support tickets.';
