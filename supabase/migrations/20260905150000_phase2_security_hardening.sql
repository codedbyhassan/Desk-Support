-- Phase 2 security hardening: server-side authorization, tenant integrity, and immutable security boundaries.

-- 1) Explicit role hierarchy. Higher rank may assign only strictly lower-ranked roles,
-- except an admin may retain the same role when updating an existing membership.
...
begin
  ...
  return v_actor_role is not null and public.can_assign_role(v_actor_role, p_target_role);
end;
$$;
...
-- Membership writes performed through an authenticated session must obey the hierarchy.
...
begin
  if auth.uid() is null then
    return new; -- Trusted service-role functions perform their own centralized checks.
  end if;
  if not public.can_actor_assign_role(new.company_id, new.role::text) then
    raise exception 'Not authorized to assign membership role';
  end if;
  return new;
end;
$$;
drop trigger if exists enforce_membership_role_change on public.company_memberships;
create trigger enforce_membership_role_change
before insert or update of role,company_id,user_id on public.company_memberships
for each row execute function private.enforce_membership_role_change();

-- 2) Last-admin and self-lockout protection at the database boundary.
create or replace function private.prevent_admin_lockout()
returns trigger language plpgsql security definer set search_path = public, private, pg_catalog as $$
declare v_active_admins integer;
begin
  if auth.uid() is null then return new; end if;
  if tg_op = 'UPDATE' and new.user_id = auth.uid() and old.is_active and not new.is_active then
  ...
  if old.is_active and not new.is_active and old.role::text = 'admin' then
    select count(*) into v_active_admins from public.company_memberships
    where company_id = old.company_id and role::text = 'admin' and is_active = true;
    if v_active_admins <= 1 then raise exception 'The company must retain at least one active administrator'; end if;
  end if;
  return new;
end;
$$;
drop trigger if exists prevent_admin_lockout on public.company_memberships;
create trigger prevent_admin_lockout before update of is_active,user_id,company_id on public.company_memberships
for each row execute function private.prevent_admin_lockout();

-- 3) Conversation member role/identity changes require conversation management.
...
begin
  ...
  if (new.role is distinct from old.role)
    ...
    if not private.can_manage_conversation(old.conversation_id, auth.uid()) then
      raise exception 'Only conversation owners/admins may change membership identity or role';
    end if;
  end if;
  return new;
end;
...
create trigger enforce_conversation_member_mutation
before update on public.conversation_members
for each row execute function private.enforce_conversation_member_mutation();

create or replace function public.manage_conversation_member(
  p_conversation_id uuid,
  p_user_id uuid,
  p_role text default 'member',
  p_new_user_id uuid default null,
  p_new_conversation_id uuid default null
) returns void language plpgsql security invoker set search_path = public, private, pg_catalog as $$
declare v_target uuid := coalesce(p_new_user_id,p_user_id); v_conversation uuid := coalesce(p_new_conversation_id,p_conversation_id);
begin
  ...
  if not private.is_conversation_member(v_conversation_id,v_target) then
    if not exists(select 1 from public.conversations c join public.company_memberships m on m.company_id=c.company_id and m.user_id=v_target and m.is_active where c.id=v_conversation_id) then
      raise exception 'Target user is not an active member of the conversation company';
    end if;
  end if;
  update public.conversation_members
  set conversation_id=v_conversation,user_id=v_target,role=p_role
...
-- 4) Call lifecycle: participants can leave their own participant row; only the initiator
-- or a company admin can end the call. Direct lifecycle column mutation is rejected.
create or replace function private.prevent_call_lifecycle_update()
returns trigger language plpgsql security definer set search_path = public, private, pg_catalog as $$
declare v_is_initiator boolean; v_is_admin boolean;
begin
  if auth.uid() is null then return new; end if;
  if new.status is distinct from old.status or new.ended_at is distinct from old.ended_at or new.end_reason is distinct from old.end_reason then
    select (c.initiator_id=auth.uid()), private.has_company_role(c.company_id,array['admin']::public.membership_role[])
      into v_is_initiator,v_is_admin from public.calls c where c.id=old.id;
    if not coalesce(v_is_initiator,false) and not coalesce(v_is_admin,false) then
      raise exception 'Only the call initiator or company administrator may change call lifecycle';
    end if;
  end if;
  return new;
end;
...
create trigger prevent_call_lifecycle_update before update on public.calls
for each row execute function private.prevent_call_lifecycle_update();
...
-- 5) Tenant-aware integrity. Existing parent PKs remain untouched; composite uniques make
-- company-aware references possible without changing public identifiers.
...
-- Tickets already carry tenant identity; department/team must match it.
drop constraint if exists tickets_department_fk on public.tickets;
...
drop constraint if exists tickets_team_fk on public.tickets;
...
-- Ticket assignments historically inherited tenant identity from the ticket. Materialize it
-- so assignee/department/team integrity can be enforced at the row boundary.
...
-- Notifications recipient must belong to the notification tenant.
...
-- Attendance user must belong to the attendance tenant.
...
-- Audit actor, when present, must be a member of the audited tenant.
...
-- QR scans historically inherited tenant identity from the QR code. Materialize it and enforce
-- that the scanning user belongs to that tenant.
...
-- Canonical calls currently use conversation_id as their contextual parent (there are no
-- team_id/ticket_id/asset_id columns in the canonical calls table). Enforce conversation tenant.
...
-- 10) Security audit: security-critical mutations are generated by the database itself.
...
create trigger audit_membership_security_change after update of role,is_active on public.company_memberships
for each row execute function private.audit_membership_security_change();

-- 11) The aggregate/count function already enforces active company membership. Preserve the
-- explicit check as a regression guard for future edits.
comment on function public.get_company_counts(uuid) is 'Tenant-scoped aggregate. Requires auth.uid() to be an active member of p_company_id.';

-- Do not validate the NOT VALID constraints here: validation must happen only after a concrete
-- production-data check. The constraints are nevertheless enforced for all new/changed rows.
