...
where check_in is not null
  and not exists (select 1 from public.attendance_sessions s where s.company_id=attendance.company_id and s.user_id=attendance.user_id and s.started_at=attendance.check_in);

-- QR scan is one transaction: authorization, validation, restrictions, attendance and log.
create or replace function public.scan_attendance_qr(p_code text,p_latitude numeric default null,p_longitude numeric default null,p_metadata jsonb default '{}'::jsonb) returns jsonb
...
begin
  ...
  select cm.company_id into v_company from public.company_memberships cm where cm.user_id=v_user and cm.is_active=true order by cm.joined_at desc limit 1;
  if v_company is null then raise exception 'Active company membership required'; end if;
  select * into v_qr from public.qr_codes q where q.code=trim(p_code) and q.company_id=v_company limit 1;
  ...
  if v_qr.status <> 'active' or (v_qr.expires_at is not null and v_qr.expires_at <= now()) then
    insert into public.qr_scan_logs(qr_code_id,user_id,result,latitude,longitude,metadata) values(v_qr.id,v_user,'invalid',p_latitude,p_longitude,coalesce(p_metadata,'{}'::jsonb)||jsonb_build_object('reason','inactive_or_expired')) returning id into v_log;
  ...
  for v_restriction in select restriction_type,value from public.qr_restrictions where qr_code_id=v_qr.id loop
    if v_restriction.restriction_type='role' then
      ...
      v_distance:=6371000*2*asin(sqrt(power(sin(radians(p_latitude::numeric-v_parts[1]::numeric)/2),2)+cos(radians(p_latitude))*cos(radians(v_parts[1]::numeric))*power(sin(radians(p_longitude::numeric-v_parts[2]::numeric)/2),2)));
      if v_distance>v_parts[3]::numeric then raise exception 'QR location restriction failed'; end if;
    else raise exception 'Unsupported QR restriction type: %',v_restriction.restriction_type;
    end if;
  end loop;
  select * into v_session from public.attendance_sessions s where s.company_id=v_company and s.user_id=v_user and s.type='work' and s.ended_at is null order by s.started_at desc limit 1;
  if found then update public.attendance_sessions set ended_at=now(),updated_at=now() where id=v_session.id returning * into v_session; v_action:='clock_out';
...
begin
  ...
  with filtered as (
    select t.* from public.tickets t where t.company_id=p_company_id and ((p_archived and t.archived_at is not null) or (not p_archived and t.archived_at is null))
      and (p_status is null or t.status::text=p_status) and (p_priority is null or t.priority::text=p_priority)
      and (p_department_id is null or t.department_id=p_department_id) and (p_team_id is null or t.team_id=p_team_id)
  ...
  select coalesce(jsonb_agg(to_jsonb(b) order by b.created_at desc,b.id desc),'[]'::jsonb),(select count(*) from filtered)>p_limit into v_items,v_has_more from bounded b;
...
-- Communication delivery idempotency and visible lifecycle.
...
alter table public.messages add constraint messages_delivery_status_check check (delivery_status in ('sending','sent','failed'));
create unique index if not exists messages_conversation_client_message_idx on public.messages(conversation_id,client_message_id) where client_message_id is not null;
...
-- Entitlements are resolved from the current subscription through explicit company feature rows.
create table if not exists public.company_entitlements(company_id uuid not null references public.companies(id) on delete cascade,feature_key text not null,enabled boolean not null default false,source_plan text,updated_at timestamptz not null default now(),primary key(company_id,feature_key));
